...
const SectionTitle = ({ children, es }) => (<div className="mb-4"><h2 className="text-xl font-bold text-slate-800 border-b-2 border-blue-600 pb-2">{children}</h2>{es && <p className="text-sm text-slate-500 mt-1">{es}</p>}</div>);

const Card = ({ title, es, children, color = "blue" }) => {
  ...
  return (<div className={`border-l-4 ${colors[color]} p-4 rounded-r-lg mb-4`}>{title && <div className="font-bold text-slate-800 mb-1">{title}</div>}{es && <div className="text-xs text-slate-500 mb-2">{es}</div>}<div className="text-sm text-slate-700">{children}</div></div>);
};
...
function InteractivePentagon() {
  ...
  const levels = [
    { key:"industry",n:4,pentIdx:2,title:"① Micro Industria",sub:"Competitive landscape — START HERE Comienza aquí",items:["Industry Characteristics","Competition","Cooperation","Strategic Groups","Lead Firms","Micro Policies","Micro Institutions"],col:PC.industry },
    { key:"meso",n:3,pentIdx:1,title:"② Meso Clúster",sub:"Cluster & value chain ecosystem",items:["Inputs & Suppliers","Demand & Customers","Shared Resources","Shared Activities","Complementarities","Substitutes","Meso Policies","Meso Institutions"],col:PC.meso },
  ...
  return (<div>
    ...
    <ISub>Orden de análisis: Industry → Meso → Macro → Meta → Firm (secuencia del profesor)</ISub>
...
function InteractiveFirmLevel() {
  ...
  const strat=[{k:"Scope Alcance",d:"Which markets, segments, geographies to compete in En qué mercados, segmentos y geografías competir"},{k:"Positioning Posicionamiento",d:"Where to position: price, performance, cost Dónde posicionar: precio, desempeño, costo"},{k:"Activities Actividades",d:"What activities to perform and how to configure them Qué actividades realizar y cómo configurarlas"},{k:"Resources Recursos",d:"What tangible/intangible assets to develop and deploy Qué activos tangibles/intangibles desarrollar y desplegar"},{k:"Knowledge Conocimiento",d:"What knowledge to create, acquire, and leverage Qué conocimiento crear, adquirir y aprovechar"},{k:"Leadership Liderazgo",d:"Who leads and how they set direction Quién lidera y cómo define la dirección"}];
  const exec=[{k:"Organization & Management Organización y Gestión",d:"Structure, processes, people, culture Estructura, procesos, personas, cultura"},{k:"Governance Gobernanza",d:"Oversight, incentives, accountability Supervisión, incentivos, rendición de cuentas"},{k:"Firm Policies Políticas de la Empresa",d:"Internal rules and guidelines Reglas y directrices internas"},{k:"Firm Institutions Instituciones de la Empresa",d:"Norms, routines, embedded practices Normas, rutinas, prácticas incorporadas"}];
  ...
  return (<div>
    ...
    <ISub>Toca cualquier ítem para ver la descripción · Tap any item for description</ISub>
    <div style={{display:"flex",flexWrap:"wrap",gap:12}}>
      ...
      <div style={{flex:"1 1 200px",minWidth:0}}>
        <div style={{fontSize:11,fontWeight:700,color:PC.firm.dark,textTransform:"uppercase",letterSpacing:1,marginBottom:8,paddingLeft:4}}>Execution Ejecución</div>
...
function InteractiveWhatIsIndustry() {
  return (<div style={{marginBottom:16}}>
    <IH>What Constitutes an Industry? ¿Qué es una Industria?</IH>
    <ISub>Definiendo los límites de la competencia Defining the boundaries of competition</ISub>
{/* ★ NEW: 3-step useful output methodology */}
    <div style={{background:"linear-gradient(135deg,#1e40af,#2563eb)",borderRadius:14,padding:"18px 16px",marginBottom:14,color:"#fff",boxShadow:"0 4px 20px rgba(37,99,235,0.2)"}}>
      <div style={{fontSize:15,fontWeight:800,textAlign:"center",marginBottom:12}}>El Método del "Output Útil" en 3 Pasos The "Useful Output" Method</div>
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {[
          {n:"1",title:"¿Qué recibe realmente el cliente?",sub:"What useful output does the customer receive?",detail:"No mires la tecnología — solo la forma y función del producto/servicio que llega al cliente.",ex:"Smartphone: comunicación móvil + apps + computación portátil"},
          {n:"2",title:"¿Quién compite directamente por ese cliente?",sub:"Who competes directly for that customer?",detail:"Todas las empresas con el mismo output útil están en la misma industria.",ex:"iOS y Android: ingeniería diferente, pero mismo output útil → misma industria"},
          {n:"3",title:"Traza el límite de la industria",sub:"Draw the industry boundary",detail:"Output diferente = industria diferente. Aunque sean 'tech'.",ex:"Amazon (logística), Microsoft (productividad), Meta (redes sociales) → 3 industrias"},
        ].map(step=>(
          <div key={step.n} style={{background:"rgba(255,255,255,0.12)",borderRadius:10,padding:"12px 14px",borderLeft:"3px solid rgba(255,255,255,0.5)"}}>
            ...
            <div style={{fontSize:11,color:"#fbbf24",paddingLeft:30,fontStyle:"italic"}}>Ej: {step.ex}</div>
          </div>
        ))}
      </div>
    </div>
    ...
    <div style={{background:"#fff",borderRadius:14,border:"2px solid #2563eb22",padding:"16px 14px",marginBottom:12,boxShadow:"0 2px 12px rgba(37,99,235,0.06)"}}>
      <div style={{display:"flex",alignItems:"center",marginBottom:10}}>
        ...
        <div style={{fontSize:15,fontWeight:700,color:"#1e40af"}}>An Industry Includes Una Industria Incluye</div>
      </div>
      <div style={{paddingLeft:36,display:"flex",flexDirection:"column",gap:6}}>
        {[<span>Products/services with <b style={{color:"#1e40af"}}>similar form and function</b> in <b style={{color:"#1e40af"}}>direct competition</b><br/><span style={{fontSize:11,color:"#64748b"}}>Productos/servicios con forma y función similares en competencia directa</span></span>,<span>The <b style={{color:"#1e40af"}}>firms</b> that provide these products and services<br/><span style={{fontSize:11,color:"#64748b"}}>Las empresas que proporcionan estos productos y servicios</span></span>].map((txt,i)=>(<div key={i} style={{fontSize:13,color:"#374151",lineHeight:1.6,paddingLeft:12,borderLeft:"3px solid #3b82f6"}}>{txt}</div>))}
      </div>
    </div>
    <div style={{background:"#fffbeb",borderRadius:14,border:"2px solid #d9770622",padding:"16px 14px",boxShadow:"0 2px 12px rgba(217,119,6,0.06)"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:5}}>
        {["Implies a particular set of customers Implica un conjunto específico de clientes","Does not generally conform to standard industry codes Generalmente no sigue códigos industriales estándar","Industry boundaries may shift over time Los límites de la industria pueden cambiar con el tiempo","Same product may appear in different industries if different customers buy for different purposes El mismo producto puede aparecer en industrias diferentes si clientes diferentes compran por propósitos diferentes"].map((txt,i)=>(<div key={i} style={{display:"flex",alignItems:"flex-start",gap:8,padding:"8px 12px",background:"#fff",borderRadius:8,border:"1px solid #fde68a"}}><Num n={i+1} color="#d97706"/><span style={{fontSize:13,color:"#4b5563",lineHeight:1.55}}>{txt}</span></div>))}
      </div>
    </div>
  </div>);
}
...
function InteractiveIndustryDetail() {
  ...
  const sections=[
    {key:"ic",title:"Industry Characteristics Características de la Industria",color:"#0369a1",border:"#bae6fd",items:["Relevant segments, activities, resources, knowledge Segmentos, actividades, recursos, conocimiento relevantes","Relevant technologies and processes Tecnologías y procesos relevantes","Geographic scope of competition Alcance geográfico de la competencia"]},
    {key:"comp",title:"Competition Competencia",color:"#b91c1c",border:"#fecaca",items:["Ferocity of competition Intensidad de la competencia","Nature of competition Naturaleza de la competencia","Identity of competitors Identidad de los competidores","Strategies of competitors Estrategias de los competidores","Capabilities of competitors Capacidades de los competidores"]},
    {key:"coop",title:"Cooperation Cooperación",color:"#059669",border:"#a7f3d0",items:["Scope for cooperation with competitors Espacio para cooperación con competidores","Alliances Alianzas","Joint development or marketing Desarrollo/marketing conjunto","Lobbying Lobby","Other joint activities Otras actividades conjuntas"]},
    {key:"sg",title:"Strategic Grouping Grupos Estratégicos",color:"#7c3aed",border:"#ddd6fe",items:["Groups of firms with similar strategies Grupos de empresas con estrategias similares","Interaction within and between groups Interacción dentro y entre grupos"]},
    {key:"lf",title:"Lead Firms Empresas Líderes",color:"#d97706",border:"#fde68a",items:["Strength Fuerza","Behavior Comportamiento"]},
  ];
  return (<div style={{marginBottom:16}}>
    <IH>Industry Drivers — Interactive Detail</IH>
    <ISub>Toca cada categoría para expandir · Tap to expand</ISub>
...
function InteractiveCompetition() {
  ...
  const types=[
    {key:"mono",label:"Monopoly Monopolio",color:"#7c3aed",items:["No competition Sin competencia","Most favorable unless limited by regulation Más favorable, salvo regulación"]},
    {key:"oligo",label:"Oligopoly Oligopolio",color:"#2563eb",items:["Competition among limited firms Competencia entre poucas empresas","Recognition of interdependence Reconocimiento de interdependencia"]},
    {key:"hyper",label:"Hypercompetition Hipercompetencia",color:"#d97706",items:["Several firms, potential new entrants Varias empresas, potenciales entrantes","Firms may distinguish themselves briefly Las empresas pueden diferenciarse brevemente"]},
    {key:"segment",label:"Segmented Competition Competencia Segmentada",color:"#475569",items:["Multiple segments with distinct buyer groups Múltiples segmentos con grupos de compradores distintos","Different price/performance packages Diferentes paquetes precio/desempeño","Pricing flexibility within segments Flexibilidad de precios dentro de los segmentos","Competition dynamics differ across segments Las dinámicas competitivas difieren entre segmentos"]},
    {key:"perfect",label:"Perfect Competition Competencia Perfeita",color:"#ea580c",items:["Many firms cannot distinguish themselves Muchas empresas no pueden diferenciarse","Price competition only Solo competencia por precio"]},
    {key:"subsid",label:"Subsidized Competition Competencia Subsidiada",color:"#dc2626",items:["Money-losing firms kept in business Empresas deficitarias mantenidas en operación","Competition on price Competencia por preço"]},
  ];
  ...
  return (<div style={{marginBottom:16}}>
    <IH>Types of Competition Tipos de Competencia</IH>
    <ISub>Toca para ver características · Tap to see characteristics</ISub>
    ...
    <div style={{background:"#fff",borderRadius:14,border:"1.5px solid #e2e8f0",padding:"18px 12px",boxShadow:"0 2px 12px rgba(0,0,0,0.04)"}}>
      <div style={{fontSize:15,fontWeight:700,textAlign:"center",color:"#0f172a",marginBottom:4}}>Where is the Industry? ¿Dónde está la Industria?</div>
      <p style={{fontSize:11,color:"#94a3b8",textAlign:"center",margin:"0 0 10px"}}>Posiciona tu industria en el espectro</p>
      <svg viewBox="0 0 500 80" style={{width:"100%",height:"auto"}}>
        ...
        <text x="30" y="70" fontSize="9" fill="#94a3b8">← Más competencia More competition</text>
        <text x="470" y="70" fontSize="9" fill="#94a3b8" textAnchor="end">Less competition Menos competencia →</text>
      </svg>
    </div>
  </div>);
}
...
function InteractiveIndustryEconomics() {
  const [open, setOpen] = useState(null);
  const questions = [
    { key:"why", title:"Why is profit POSSIBLE here? ¿Por qué la ganancia es posible aquí?", color:"#059669", border:"#a7f3d0",
      items:[
        "Which departures from perfect competition exist? ¿Qué desviaciones de competencia perfecta?",
        "Entry barriers: scale, learning, scope, brands, patents, regulation, retaliation ¿Barreras de entrada?",
        "Exit barriers: specialized assets, strategic/emotional barriers, exit costs ¿Barreras de salida?",
        "Information asymmetries between firms and/or customers ¿Asimetrías de información?",
        "Are products differentiable or homogeneous? ¿Diferenciables u homogéneos?",
      ]},
    { key:"where", title:"Where does profit COME FROM? ¿De dónde viene la ganancia?", color:"#2563eb", border:"#bfdbfe",
      items:[
        "Price premiums from differentiation (brand, quality, features)? ¿Primas por diferenciación?",
        "Cost advantages from scale, learning, scope, or resource access? ¿Ventajas de costo por escala?",
        "Customer switching costs or lock-in? ¿Costos de cambio?",
        "Regulatory protection or government policies? ¿Protección regulatoria?",
        "Network effects or platform dynamics? ¿Efectos de red?",
        "Control of scarce inputs, distribution, or complementary assets? ¿Control de insumos escasos?",
      ]},
    { key:"shift", title:"What SHIFTS would change the profit structure? ¿Qué cambiaría la estructura?", color:"#dc2626", border:"#fecaca",
      items:[
        "New entrants overcoming barriers (technology change, regulation change)? ¿Nuevos entrantes superando barreras?",
        "Substitutes emerging from adjacent industries? ¿Sustitutos adyacentes?",
        "Buyer or supplier power shifting (consolidation, vertical integration)? ¿Cambio en poder de compradores?",
        "Competition type migrating on the spectrum (e.g., oligopoly → hypercompetition)? ¿Tipo de competencia migrando?",
        "Macro/Meta forces disrupting the structure (trade policy, technology waves, geopolitics)? ¿Fuerzas macro/meta perturbando?",
        "Lead firms changing strategy or new lead firms emerging? ¿Empresas líderes cambiando?",
      ]},
  ];

  return (<div style={{marginBottom:16}}>
    <IH>Industry Economics Economía de la Industria</IH>
    <ISub>No es estática — es cómo la estructura de ganancia se forma, persiste y cambia</ISub>
    <div style={{background:"#fef2f2",border:"2px solid #dc262622",borderRadius:14,padding:"12px 14px",marginBottom:12}}>
      <div style={{fontSize:13,fontWeight:700,color:"#991b1b",textAlign:"center",marginBottom:4}}>⚠️ Error Común Common Mistake</div>
      <div style={{display:"flex",gap:8,flexWrap:"wrap",justifyContent:"center"}}>
        <div style={{background:"#fff",border:"1px solid #fca5a5",borderRadius:8,padding:"6px 12px",fontSize:12}}><span style={{color:"#dc2626",fontWeight:700}}>✗</span> Tratar como '¿cuál es el margen actual?'</div>
        <div style={{background:"#fff",border:"1px solid #86efac",borderRadius:8,padding:"6px 12px",fontSize:12}}><span style={{color:"#16a34a",fontWeight:700}}>✓</span> Preguntar '¿Por qué posible? ¿De dónde? ¿Qué cambiaría?'</div>
      </div>
    </div>
    <div style={{display:"flex",flexDirection:"column",gap:8}}>
...
function Overview() {
  return (<div>
    <SectionTitle es="Información del examen y cómo responder">Exam Info & How to Write</SectionTitle>
    <Card title="Exam Structure (from Practice Exam)" es="Estructura del examen (del simulacro)" color="blue">
      ...
      <div className="bg-amber-50 border border-amber-300 rounded p-2 text-xs">⚠️ Total = 75%. Espera una <strong>4ª pregunta no revelada (25%)</strong>. Prepárate para aplicar Five Levels + SPARK a un escenario desconocido.</div>
    </Card>
    <Card title="The Single Governing Logic" es="La lógica central única del curso" color="green">
      <div className="bg-white rounded-lg p-4 text-center border">
        <div className="text-lg font-bold text-green-800 mb-2">Understand and improve firm performance</div>
        <div className="text-sm text-slate-600 mb-3">Comprender y mejorar el desempeño de la empresa</div>
        <div className="flex flex-wrap justify-center gap-2">
          {[["Performance is RELATIVE","Relativo"],["Comprehensive","Integral"],["Integrative","Integrativo"],["Dynamic","Dinámico"],["Question-based","Basado en preguntas"]].map(([e,v])=>(<Tag key={e} color="green">{e} {v}</Tag>))}
        </div>
      </div>
    </Card>
    <Card title="Strategy in This Course (Ch.1)" es="Definición de estrategia en este curso" color="purple">
      ...
      <p className="mt-1 text-xs text-center text-slate-400">No hay "balas mágicas." La estrategia incluye tanto la visión general COMO los planes detallados para la ejecución.</p>
    </Card>
    <Card title="6-Step Answer Formula" es="Fórmula de respuesta en 6 pasos (inferida de las exigencias del material)" color="amber">
      <div className="space-y-2">
        {[["1","State the performance issue","Plantea el problema de desempeño"],["2","Identify the dominant level","Identifica el nivel dominante"],["3","Name specific drivers","Nombra los drivers específicos"],["4","Explain HOW → mechanism","Explica el mecanismo de impacto"],["5","Cross-level linkage","Conexión entre niveles"],["6","Judge: positive/negative? Persist?","Juzga: ¿positivo/negativo? ¿Persiste?"]].map(([n,en,pt])=>(<div key={n} className="flex items-start gap-2"><div className="bg-amber-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs font-bold flex-shrink-0">{n}</div><div><span className="font-semibold text-sm">{en}</span> <span className="text-xs text-slate-500">{pt}</span></div></div>))}
      </div>
    </Card>
    <Card title="Weak vs. Strong" es="Respuesta débil vs. fuerte" color="red">
...
function FiveLevels() {
  return (<div>
    <SectionTitle es="El Marco de Cinco Niveles — Diagramas interactivos + Referencia">The Five-Level Framework</SectionTitle>
    <div className="mb-6"><InteractivePentagon /></div>
    <Card title="Three Critical Insights (Ch.2)" es="Tres ideas fundamentales" color="blue"><div className="grid grid-cols-3 gap-2">{[["Systemic Sistémico","Levels interdependent; changes cascade"],["Changing Cambio continuo","Must project forward, not snapshot"],["Interdependent Interdependiente","Favorable macro ≠ firm success if micro/firm unfavorable"]].map(([t,d])=>(<div key={t} className="bg-white border rounded p-3 text-center"><div className="font-bold text-blue-700 text-sm mb-1">{t}</div><div className="text-xs text-slate-600">{d}</div></div>))}</div></Card>
    <Card title="Levels AND Trends (Ch.2)" es="Niveles Y Tendencias" color="amber"><div className="text-center font-semibold">"Better might still not be good. Worse might still not be bad."</div><div className="text-center text-xs text-slate-500 mt-1">Mejorar no significa necesariamente que esté bien. Empeorar no significa necesariamente que esté mal.</div></Card>
    <Card title="Q1 Template" es="Plantilla de respuesta rápida para Q1" color="green"><div className="bg-white border rounded p-3 text-sm italic">"At the [level], the crucial driver was [X]. This mattered because [mechanism]. It influenced profitability by [price / cost / demand / bargaining / entry / rivalry]. The impact was [positive / negative], and the trend was [direction]."</div><div className="mt-2 bg-red-50 border border-red-300 rounded p-2 text-xs">⚠️ Q1 requiere 5 casos diferentes, uno por nivel. <strong>Completa con tus notas de las Sessions 1–8.</strong></div></Card>
  </div>);
}

function SparkPosition() {
  return (<div>
    <SectionTitle es="Modelo SPARK y Análisis de Posicionamiento">SPARK Model & Positioning</SectionTitle>
    ...
    <div className="bg-gradient-to-br from-green-800 to-green-900 text-white rounded-xl p-5 mb-4">
      <div className="text-center font-bold text-xl mb-1">Strategy = SPARK</div><div className="text-center text-green-300 text-xs mb-4">Ch.3 — Herramienta de análisis a nivel empresa</div>
      <div className="space-y-2">{[{l:"S",w:"cope",q:"WHERE? ¿Dónde?",d:"Industries, segments, geography",c:"bg-green-700"},{l:"P",w:"ositioning",q:"HOW? ¿Cómo?",d:"Price/performance + cost + vs. competitors",c:"bg-green-600"},{l:"A",w:"ctivities",q:"DO what? ¿Qué hace?",d:"Tasks to serve customers",c:"bg-emerald-700"},{l:"R",w:"esources",q:"HAVE what? ¿Qué tiene?",d:"Brands, patents, workforce, facilities, financial",c:"bg-emerald-600"},{l:"K",w:"nowledge",q:"KNOW what? ¿Qué sabe?",d:"Market, tech, competitor, process, organizational",c:"bg-teal-700"}].map(s=>(<div key={s.l} className={`${s.c} rounded-lg p-3 flex items-center gap-3`}><div className="text-3xl font-black text-green-200 w-8">{s.l}</div><div className="flex-1"><div className="flex items-baseline gap-2"><span className="font-bold">{s.l}<span className="font-normal">{s.w}</span></span><span className="text-green-300 text-xs">{s.q}</span></div><div className="text-xs text-green-200 mt-0.5">{s.d}</div></div></div>))}</div>
...
function EtaSwatch() {
  return (<div>
    ...
    <div className="grid grid-cols-2 gap-3 mb-4">
      ...
      <Card title="D. Movement vs Watch economics?" color="purple"><div className="text-xs space-y-1"><div><strong>Movements:</strong> High fixed cost, massive scale → few firms → oligopoly</div><div><strong>Watches:</strong> Brand differentiation, many segments → many firms → segmented</div><div className="font-semibold mt-1">Movimientos → concentración natural. Relojes → fragmentación natural.</div></div></Card>
    </div>
    <Card title="Movement vs Watch Economics" es="Comparación económica: movimientos vs. relojes" color="slate"><table className="w-full text-xs"><thead><tr><th className="text-left pb-1"></th><th className="text-left pb-1 text-blue-700">Movements</th><th className="text-left pb-1 text-amber-700">Watches</th></tr></thead><tbody className="divide-y">{[["Scale economies","Very high (5yr, CHF 10M; billions)","Lower (assembly/branding)"],["Entry barriers","Very high","Lower with movement access"],["Viable firms","Few → oligopoly","Many → segmented"],["Value capture","Manufacturing efficiency + scarcity","Brand + design + customer rel."]].map(([f,m,w])=>(<tr key={f}><td className="py-1 pr-2 font-semibold">{f}</td><td className="py-1 pr-2">{m}</td><td className="py-1">{w}</td></tr>))}</tbody></table></Card>
  </div>);
}

function CramSheet() {
  return (<div>
    <SectionTitle es="Resumen rápido y checklist final">Cram Sheet & Final Checklist</SectionTitle>
    <div className="bg-slate-900 text-white rounded-xl p-5 mb-4"><div className="text-center font-bold text-xl mb-4 text-yellow-300">17 THINGS TO KNOW COLD</div><div className="space-y-2">{[["1","Performance is RELATIVE","El desempeño es RELATIVO","blue"],["2","Comprehensive, integrative, dynamic, question-based","Integral, integrativo, dinámico, basado en preguntas","blue"],["3","Five levels: Industry → Meso → Macro → Meta → Firm","Orden: Industria→Clúster→Nacional→Supra→Empresa","blue"],["4","Levels AND trends","Niveles Y tendencias","blue"],["5","Industry = useful output to customer + direct competition","Industria = output útil + competencia directa","amber"],["6","Full positioning = price AND cost","Posicionamiento completo = precio Y costo","green"],["7","SPARK: Scope, Positioning, Activities, Resources, Knowledge","","green"],["8","VRIO extends to ARK in SPARK","VRIO se extiende al ARK en SPARK","purple"],["9","Complementors EXPAND; substitutes CONTRACT demand","Complementarios expandem; substitutos contraem demanda","cyan"],["10","Ecosystems: where is value generated, appropriated, defended?","Ecosistemas: ¿dónde se genera, apropia y defiende el valor?","red"],["11","Macro: levels, trends, disruption, non-linearity","Macro: niveles, tendencias, disrupción, no linealidad","blue"],["12","Q2 Home Alone: THE PART → THE KID → THE FRANCHISE","","green"],["13","Q3 ETA: 5yr, CHF 10M, 33% by 2019, more concentrated","","amber"],["14","Barriers to entry/exit allow profit differences to PERSIST","Las barreras hacen que las diferencias de ganancia persistan","purple"],["15","Industry Economics: WHY possible, WHERE from, WHAT shifts it","Economía: por qué posible, de dónde, qué cambia","rose"],["16","General vs. Specific advantages → explains hustle logic","Ventajas generales vs. específicas","cyan"],["17",`"Wonderful Life" test: remove a player, does it matter?`,"Elimina a un participante, ¿importa?","purple"]].map(([n,en,pt,c])=>{const colors={blue:"bg-blue-800",green:"bg-green-800",amber:"bg-amber-800",purple:"bg-purple-800",red:"bg-red-800",cyan:"bg-cyan-800",rose:"bg-rose-800"};return(<div key={n} className={`${colors[c]} rounded-lg px-4 py-2 flex items-center gap-3`}><div className="bg-white text-slate-900 rounded-full w-6 h-6 flex items-center justify-center text-xs font-bold flex-shrink-0">{n}</div><div className="flex-1"><span className="font-semibold text-sm">{en}</span>{pt&&<span className="text-xs text-slate-300 ml-2">{pt}</span>}</div></div>);})}</div></div>
    <Card title="Mistakes That Cost Points" es="Errores que más cuestan puntos" color="red"><div className="grid grid-cols-2 gap-2 text-xs">{["Industry by tech, not useful output","Performance as absolute, not relative","Static analysis, no trends","Levels listed without HOW/WHY mechanism","Meso confused with micro",'"Better resources" without WTP/cost effect',"Positioning from price alone or cost alone","Unsupported case detail not in files","Industry economics as snapshot, not dynamic structure"].map(m=>(<div key={m} className="flex items-start gap-1"><span className="text-red-500 flex-shrink-0">✗</span><span>{m}</span></div>))}</div></Card>
    <Card title="Final 60-Minute Review Plan" es="Plan de repaso de los últimos 60 minutos" color="green"><div className="space-y-3">{[{t:"0–15 min",a:"MEMORIZE Memoriza",d:"17 items above. Performance is relative. Five levels + drivers. SPARK. ARK. Price + cost."},{t:"15–30 min",a:"WRITE FROM MEMORY Escribe de memoria",d:"Five levels + drivers. Competition spectrum. Home Alone: part/kid/franchise. ETA facts."},{t:"30–45 min",a:"PRACTICE 3 MINI-ANSWERS Practica 3 respuestas",d:"One Five-Level. One VRIO/ARK. One ETA. Each 6–8 sentences."},{t:"45–60 min",a:"CHECK TWO THINGS Verifica dos cosas",d:"Did I explain HOW? Did I explain WHY?"}].map(p=>(<div key={p.t} className="bg-green-50 border border-green-200 rounded-lg p-3"><div className="flex items-center gap-2 mb-1"><Tag color="green">{p.t}</Tag><span className="font-bold text-green-800 text-sm">{p.a}</span></div><div className="text-xs text-slate-700">{p.d}</div></div>))}</div></Card>
    <Card title="Final Checklist" es="Checklist final" color="amber"><div className="space-y-2 text-xs">{["Identify 3 more cases from Sessions 1–8 for Q1 (biggest gap)","Prepare for unknown Q4 (Five Levels + SPARK on unfamiliar scenario)","Write from memory: five levels + drivers + SPARK + competition types + HA sequence + ETA facts","Practice 3 mini-answers (one per question type)","Every answer: HOW? and WHY?"].map((c,i)=>(<div key={i} className="flex items-start gap-2"><div className="w-4 h-4 border-2 border-amber-400 rounded flex-shrink-0 mt-0.5"></div><span>{c}</span></div>))}</div></Card>
    <div className="bg-slate-100 rounded-lg p-4 text-center"><div className="font-bold text-slate-800 text-sm mb-1">The reflex to bring into the exam room:</div><div className="text-slate-700 text-sm"><strong>Identify the level → Identify the driver → Explain the mechanism → Judge the performance effect</strong></div><div className="text-xs text-slate-500 mt-1">Identifica el nivel → Identifica el driver → Explica el mecanismo → Juzga el efecto en el desempeño</div></div>
...
export default function App() {
  ...
  return (
    <div className="bg-white min-h-screen">
      <div className="bg-slate-900 text-white px-4 py-3"><div className="text-lg font-bold">STRT 6200 Midterm Study Guide</div><div className="text-xs text-slate-400">Enhanced Edition · English - Español Mexicano | Closed Book</div></div>
